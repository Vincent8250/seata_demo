...
- 业务悬挂：对于回滚的业务 如果后续还有try要执行 要保证后续的try不能confirm或cancel
- 空回滚：可能因为某个try阶段阻塞导致后续try未执行 就进入到二阶段的cancel中
  未执行try就进行了cancel是错误的 所以需要空回滚处理这种问题
- 幂等性：就是无论接口调用多少次 返回的接口应该具有一致性

TCC的try、confirm、cancel方法都需要在接口中基于注解来声明

1. 首先声明接口 标明TCC的三个方法
   ![image-20230720230541429](img/image-20230720230541429.png)
2. 然后实现TCC接口
3. 需要在cancel逻辑中实现空回滚逻辑
4. 还需要在confirm、cancel逻辑中保证幂等性
5. 还需要保证try逻辑中的业务悬挂
